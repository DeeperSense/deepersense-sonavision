...
def PIX2PIX_GENERATOR(
    ...
    if "with-camera-" in arch_type:
        ...
        return tf.keras.Model(inputs=sonar_input, outputs=output)
...
def PIX2PIX_DISCRIMINATOR(
    ...
    if "with-camera-" in arch_type:
        ...
        return tf.keras.Model(inputs=[sonar_input, reference_input], outputs=last)
