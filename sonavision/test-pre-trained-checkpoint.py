...
def generate_images(
    ...
    if how_to_save == "detailed" or how_to_save == "both":
        path = timestamp + "/results-chkpt-23/" + dark_mode + "/detailed/"
    ...
    if how_to_save == "concate" or how_to_save == "both":
        path = timestamp + "/results-chkpt-23/" + dark_mode + "/concate/"
        if not os.path.isdir(path):
            os.makedirs(path)
...
chkpt_dir = "{}/checkpoints".format(timestamp)
chkpt_name = "/ckpt-23"
batch_size = 1
...
checkpoint.restore(chkpt_dir+chkpt_name).expect_partial()
print("[INFO] Checkpoint restored", chkpt_dir+chkpt_name)
